...
import { useAcuerdosRelevantes } from '@/hooks/useAcuerdosRelevantes';
import { formatFecha } from '@/lib/format';
import { formatCurrency } from '@/lib/formatters';

export function AcuerdosRelevantesTable() {
  const { data: relevantes } = useAcuerdosRelevantes();
          ...
          <tbody>
            {relevantes.map((acuerdo) => (
              <tr key={acuerdo.id} className="border-b border-gray-50 last:border-0">
                ...
                <td className="py-2.5 pr-3 text-gray-600">{acuerdo.marcaNombre}</td>
                ...
                <td className="py-2.5 pr-3">
                  <div className="flex items-center gap-2">
                    <ProgressBar valor={acuerdo.cumplimiento} className="w-20" />
                    <span className="text-xs text-gray-500">{acuerdo.cumplimiento}%</span>
                  </div>
                </td>
                <td className="py-2.5">
