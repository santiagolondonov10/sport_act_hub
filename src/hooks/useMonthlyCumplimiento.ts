...
export function useMonthlyCumplimiento() {
  const [data, setData] = useState<CumplimientoMensual[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setError('No fue posible cargar los datos de cumplimiento.');
          setData([]);
        }
      } catch (err) {
        console.error('Error fetching monthly compliance:', err);
