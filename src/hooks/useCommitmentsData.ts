...
export function useCommitmentsData() {
  const [data, setData] = useState<EstadoCompromisoResumen[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await apiCall('/api/dashboard/compromisos-por-estado', { headers });
        if (response.ok) {
          const data = await response.json();
          setData(data);
        } else {
          setError('No fue posible cargar los datos de compromisos.');
          setData([]);
        }
      } catch (err) {
