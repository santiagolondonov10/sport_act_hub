                        ...
                        : filename.startsWith('022') && filename.includes('fotos')
                          ...
                          : filename.startsWith('025')
                            ...
                            : filename.startsWith('023')
                              ...
                              ? false
                              : filename.startsWith('040')
                              ? !(await client.query(`SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'compromisos' AND column_name = 'marca_id') AS exists`)).rows[0].exists
                              : filename.startsWith('041')
                              ? (await client.query(`SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'evidencias' AND column_name = 'url') AS exists`)).rows[0].exists
